# Changelog

## [0.2.2](https://github.com/joshtgl/ashsa/compare/v0.2.1...v0.2.2) - 2026-06-15

### Added

- add support for attestation and SBOM

### Other

- fix SBOM naming
- create SBOM in lambda build

## [0.2.1](https://github.com/joshtgl/ashsa/compare/v0.2.0...v0.2.1) - 2026-06-11

### Fixed

- do not use ansi in lambda for logging
- long lived token takes precedence if specified

### Other

- Update github actions to v0.5.130
- Update github actions to v6.0.3

## [0.2.0](https://github.com/joshtgl/ashsa/compare/v0.1.0...v0.2.0) - 2026-06-02

### Added

- [**breaking**] allow long lived token to be used even if debug is not set

### Other

- add default git-cliff config
- fix env vars, use GITHUB_TOKEN
- use release-plz for the release process

## [0.1.0] - 2026-06-01

- Initial release.
